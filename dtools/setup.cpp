...
/**
 * @brief dxsinglemotorSetup
 * Sets up a single-motor system and locks its EEPROM
 * @param deviceIndex Port number of the USB/serial device
 * @return Pointer to the DXSingleMotor object
 */
DXSingleMotor* dxsinglemotorSetup(const int deviceIndex)
{
    DXSingleMotor* d1 = new DXSingleMotor(deviceIndex,BAUDRATE);
    // Open USB2Dynamixel //
    if (!d1->isInitialized())
    {
        throw runtime_error("Failed to open USB2Dynamixel!");
    }
    cout << "Succeed to open USB2Dynamixel!" << endl;
    if (!d1->motorInitialized())
    {
        throw runtime_error("There are no motors connected.");
    }
